...
var data=[];var MatrixC=[];var X;var temp=[];
var columns = [
    {
        title: "Iteration",
        dataIndex: "iteration",
 ...
 if (data !== undefined) {
  ...
  this.setState({exampledata:data})
   ...
   createMatrix = () => {
    ...
    while(iteration < 4){
      yyy = [];
        for(var i=0;i!=this.state.valuesubmit;i++){  
       
          bi = B._data[i];
        
          for(var j=0;j!=this.state.valuesubmit;j++){
            if(i != j){
              console.log()
              console.log()
              console.log()
              console.log()
                bi = bi-(A._data[i][j]*allans[j]);
            }
          }
          bi = bi/A._data[i][i];
          
          error = math.abs((bi-allans[i])/bi);
          error = error.toFixed(6)
          console.log("x",i, 'value=',bi,'Iteration=',iteration,'error=',error)
          allans[i] = bi;
          bi = 0;
          if(error <= 0.0000001){
            break;
...
export default withRouter(Jacobi)
