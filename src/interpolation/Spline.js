...
import * as cubic from 'cubic-spline';
// import axios from "axios";
...
class Spline extends React.Component {  
 constructor(props) {
    ...
    spline() {
      ...
      // let a = parseFloat(document.getElementById("x" + (i ) + "" + (j )).value);
          for (let i = 0; i !=this.state.valuesubmit; i++) {
              ...
              xArr[i ] = b;
              yArr[i ] = c;console.log(b);console.log(c)
          }      
          var MM = new cubic(xArr, yArr);        
          console.log(MM.at(this.state.xi)); 
          document.getElementById('ans').innerHTML=MM.at(data.Xi);
    ...
    handleSubmit2 = (event) => {
        event.preventDefault()
        this.setState({HelperText:''})
        this.setState({xi:document.getElementById('xx').value})
    ...
    handleSubmit3 = async (event) => {
          ...
          this.setState({valuesubmit:this.state.exampledata.Xi})
          ...
          document.getElementById('xx').value=this.state.valuesubmit
          var MM = new cubic(data.X, data.Y);        
  ...
  return (
      ...
      <h1 style={{textAlign:'center'}}>Spline</h1>
...
export default withRouter(Spline)
